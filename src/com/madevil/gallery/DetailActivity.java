...
public class DetailActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		DataPicture picture = (DataPicture) intent.getSerializableExtra(DataPicture.intentTag);
		mImageFetcher.loadImage(picture.getUrl(), (ImageView) findViewById(R.id.detail_image));

		Button buttonLike = (Button) this.findViewById(R.id.detail_btn_like);
		Button buttonComment = (Button) this.findViewById(R.id.detail_btn_comment);
		Button buttonDownload = (Button) this.findViewById(R.id.detail_btn_download);
		buttonLike.setText(String.format("%s", picture.getLikeNumber()));
		buttonComment.setText(String.format("%s", picture.getCommentNumber()));
		buttonDownload.setText(String.format("%s", picture.getDownloadNumber()));
	...
	private class TaskGetDetail extends AsyncTask<String, Integer, Integer> {
		...
		msg = "图片服务器返回的数据为空！请休息一下再来～";
			...
			msg = "" + ecode + "." + json_root.optString("msg", "系统繁忙，请休息一下再来～");
			return ecode;
		}
